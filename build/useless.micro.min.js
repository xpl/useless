...
0<e.precision?_.toFixed(a,e.precision):a+"";var h=_.isArray(a),f=e.pretty||!1;if(a.toJSON)return _.quoteWith('"',a.toJSON());if(!e.pure&&(d>(e.maxDepth||5)||h&&a.length>(e.maxArrayLength||30)))return h?"<array["+a.length+"]>":"<object>";var k=b.concat([a]);c.push(a);b=_.pairs(a);var m=!f||2>b.length,l=g+1,n=m?"":"\t".repeats(l);if(f&&!h){var p=_.reduce(_.map(_.keys(a),_.count),_.largest,0);b=_.map(b,function(a){return[a[0],a[1]," ".repeats(p-a[0].length)]})}return _.quoteWith(h?m?"[]":"[\n  ]":m?
...
_.clamp=function(a,b,c){return Math.max(b,Math.min(c,a))};_.lerp=function(a,b,c){return b+(c-b)*a};_.rescale=function(a,b,c,d){a=(a-b[0])/(b[1]-b[0]);return _.lerp(d&&d.clamp?_.clamp(a,0,1):a,c[0],c[1])};_.sqr=function(a){return a*a};Math.sign||(Math.sign=function(a){return 0>a?-1:0<a?1:0});
...
"translate("+a.x+","+a.y+")")},transform:function(a){a=a.components;return this.attr("transform","matrix("+a[0][0]+","+a[1][0]+","+a[0][1]+","+a[1][1]+","+a[0][2]+","+a[1][2]+")")},clientBBox:function(){var a=this[0].getBoundingClientRect();return new BBox(a.left,a.top,a.width,a.height)}})}(jQuery);
