...
e){var g=b[d];_.isBindable(g)&&_.each(a,function(a){g["_"+a]=_.without(g["_"+a],e)})},isBindable:function(a){return a&&a._bindable?!0:!1},bindable:_.extendWith({hooks:a,hooksShort:["onceBefore","onceAfter","before","after",""]},function(a,d){return _.withSameArgs(a,_.extendWith(b(a),function(){var b=arguments.callee,g=b._onceBefore,f=b._onceAfter,h=b._onBefore,k=b._onAfter,m=b._intercept,b=d||this,l,n=void 0;if(g.length){l=0;for(n=g.length;l<n;l++)g[l].apply(b,arguments);g.removeAll()}l=0;for(n=h.length;l<
n;l++)h[l].apply(b,arguments);g=(m.length?_.cps.compose([a].concat(m)):a).apply(b,arguments);if(k.length||f.length){h=_.asArray(arguments).concat(g);l=0;for(n=k.length;l<n;l++)k[l].apply(b,h);if(f.length){l=0;for(n=f.length;l<n;l++)f[l].apply(b,h);f.removeAll()}}return g}))})})})();
...
this.bottom),f=new Vec2(this.left,this.bottom),c=[a.projectOnLineSegment(d.add(c,0),e.add(-c,0)),a.projectOnLineSegment(e.add(0,c),g.add(0,-c)),a.projectOnLineSegment(g.add(-c,0),f.add(c,0)),a.projectOnLineSegment(f.add(0,-c),d.add(0,c)),a.projectOnCircle(d.add(c,c),c),a.projectOnCircle(e.add(-c,c),c),a.projectOnCircle(g.add(-c,-c),c),a.projectOnCircle(f.add(c,-c),c)];return _.min(c,function(b){return a.sub(b).length})},xywh:$property(function(){return{x:this.x,y:this.y,width:this.width,height:this.height}}),
