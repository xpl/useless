...
a instanceof $global[c]&&c+" "+_.stringifyOneLine(_.omitTypeMismatches(b,a))||!1}))return f;var g=_.isArray(a),f=e.pretty||!1;if("browser"===_.platform().engine){if(_.isTypeOf(Element,a))return a.tagName.lowercase.quote("<>");if(_.isTypeOf(Text,a))return"@"+a.wholeText}if(a.toJSON)return _.quoteWith('"',a.toJSON());if(!e.pure&&(d>(e.maxDepth||5)||g&&a.length>(e.maxArrayLength||30)))return g?"<array["+a.length+"]>":"<object>";var h=b.concat([a]);c.push(a);var k=_.pairs(a),m=_.stringifyImpl.tails2(h,
c,d+1,e);if(f){k=_.values(a);d=_.alignStringsRight(_.keys(a).map(_.appends(": ")));e=k.map(m);var f=e.map(function(a,b){return"["===a[0]||"{"===a[0]?3:_.isString(k[b])?1:0}),l=_.max(f);e=[f,e].zip(function(a,b){return" ".repeats(l-a)+b});d=g?e:[d,e].zip(_.bullet);d=_.bullet(g?"[ ":"{ ",d.join(",\n"));e=d.split("\n");return d+(" ".repeats(_.max(e.map(_.count))-_.count(e.last))+(g?" ]":" }"))}return _.quoteWith(g?"[]":"{  }",_.joinWith(", ",_.map(k,function(a){return(g?"":a[0]+": ")+m(a[1])})))};
