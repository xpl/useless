...
c,d+1,e);if(f){k=_.values(a);d=_.alignStringsRight(_.keys(a).map(_.appends(": ")));e=k.map(l);var f=e.map(function(a,b){return"["===a[0]||"{"===a[0]?3:_.isString(k[b])?1:0}),n=_.max(f);e=[f,e].zip(function(a,b){return" ".repeats(n-a)+b});d=g?e:[d,e].zip(_.bullet);d=_.bullet(g?"[ ":"{ ",d.join(",\n"));e=d.split("\n");return d+(" ".repeats(_.max(e.map(_.count))-_.count(e.last))+(g?" ]":" }"))}return _.quoteWith(g?"[]":"{  }",_.joinWith(", ",_.map(k,function(a){return(g?"":a[0]+": ")+l(a[1])})))};
...
a('<span class="file">').text(b.fileName),a('<span class="line">').text(b.line)])},printFailedTest:function(b){var c=a('<pre class="test-log" style="margin-top: 13px;">');log.withWriteBackend(this.$(function(b){_.isTypeOf(Error,b.args.first)&&console.log(b.args.first);c.append(_.isTypeOf(Error,b.args.first)?a("<div>").attr("style",b.color&&b.color.css||"").append([_.escape(b.indentation),a('<div class="panic-alert-error inline-exception all-stack-entries">').append(this.printError(b.args.first))]):
