...
Date.now();return this.on("touchend",function(){var e=Date.now();200>e-b&&a.apply(this,arguments);b=e})}return this.dblclick(a)},nodoubletapzoom:function(){return a(this).bind("touchstart",function(b){var d=b.timeStamp,e=a(this).data("lastTouch")||d,e=d-e,f=b.originalEvent.touches.length;a(this).data("lastTouch",d);!e||500<e||1<f||(b.preventDefault(),a(b.target).trigger("click"))})}})}(jQuery);_.defineTagKeyword("required");_.defineTagKeyword("atom");_.defineKeyword("any",_.identity);
...
this.btnClose=a('<button type="button" class="panic-btn panic-btn-danger" style="display:none;">Close</button>').touchClick(this.close)])])]);b.appendTo(document.body);try{this.initAutosize(),this.modal.enableScrollFaders({scroller:this.modalBody}),a(document).keydown(this.$(function(a){27===a.keyCode&&this.close()}))}catch(c){_.delay(function(){Panic(c)})}return b})),initAutosize:function(){a(window).resize(this.$(function(){this.modal.css("max-height",a(document).height()-100);this.modalBody.scroll()})).resize()},
...
"").click(this.$(function(b){a(b.delegateTarget).parent().toggleClass("all").transitionend(this.$(function(){this.modalBody.scroll()}))})),a('<ul class="callstack">').append(_.map(c,this.$(function(b){var c=a('<li class="callstack-entry">').toggleClass("third-party",b.thirdParty).toggleClass("native",b["native"]).append([a('<span class="file">').text(_.nonempty([b.index?"(index)":b.fileShort,b.line]).join(":")),a('<span class="callee">').text(b.calleeShort),a('<span class="src i-am-busy">').click(this.$(function(d){var h=
a(d.delegateTarget);h.waitUntil(_.readSource.partial((b.remote?"api/source/":"")+b.file),this.$(function(d){c.is(".full")?(c.removeClass("full"),c.transitionend(function(){c.is(".full")||h.text(b.source)})):(c.addClass("full"),h.html(_.map(d.split("\n"),function(b){return a('<div class="line">').text(b)})),d=h.find(".line").eq(b.line-1).addClass("hili"),d.length&&(d=d.offset().top-h.offset().top,h.scrollTop(d-100)),_.delay(this.$(function(){var a=h.outerBBox().bottom+242-this.modalBody.outerBBox().bottom;
