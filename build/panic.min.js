...
(function(){Array.prototype.mergeSort=function(a){var b=Math.floor(this.length/2);if(1>b)a=this;else{for(var c=this.slice(0,b).mergeSort(a),b=this.slice(b).mergeSort(a),d=[];c.length&&b.length;)d.push(0>a(c[0],b[0])?c.shift():b.shift());a=d.concat(c.length?c:b)}return a};Array.prototype.squash=function(a){a=a||{};a=a.key||_.identity;for(var b=[],c={},d=0,e=this.length;d<e;d++){for(var f=this[d],g={},h=0,k=f.length;h<k;h++){var l=f[h],m=a(l);c[m]=l;g[m]=h}b.push(g)}f=_.keys(c).mergeSort(function(a,
c){if(a===c)return 0;for(var d=0,e=0,f=0,g=0,h=b.length,k,l;g<h;g++){var m=b[g];void 0!==(k=m[a])&&void 0!==(l=m[c])&&(k<l?d++:k>l?e++:f++)}return d>e?1:d<e?-1:0}).reverse();d=0;for(e=f.length;d<e;d++)a=f[d],f[d]=c[a];return f}})();$global.define("DAG",{squash:function(a,b){b=b||{};var c=b.items||_.noop,d=function(a,b){b=b||[];(ix=c(a))&&ix.length&&(ix.reduce(function(a,c){b.push([c,a]);return c}),ix.forEach(function(c){b.push([a,c]);d(c,b)}));return b};return d(a).squash(b).remove(a).reverse()}});
