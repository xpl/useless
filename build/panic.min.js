...
_.defineKeyword("interlocked",function(a){var b=new Lock;return _.prependsArguments(Tags.unwrap(a),function(a){b.acquire(function(){a(b.$(b.release))})})});_.defineKeyword("scope",function(a){var b=void 0;return _.prependsArguments(Tags.unwrap(a),function(a){var d={when:void 0};(b=b||[]).push(d);a(function(a){if(d.when)throw Error("$scope: release called twice");for(d.when=a;b&&b.last&&b.last.when;)a=b.last.when,(b=_.initial(b)).isEmpty&&(b=void 0),a()})})});Platform.NodeJS&&(module.exports=_);
...
log.impl.defaultWriteBackend},impl:{write:function(a){return $restArg(function(){var b=log.writeBackend(),c=_.asArray(arguments),d=log.cleanArgs(c),e=_.extend({indent:0},a,log.readConfig(c)),f=(b.indent||0)+e.indent,g=log.impl.stringifyArguments(d,e),f=_.times(f,_.constant("\t")).join(""),g=g.reversed.match(/(\n*)([^]*)/),h=e.location&&log.impl.location(e.where||$callStack[3+(e.stackOffset||0)])||"",c={color:log.readColor(c),indentedText:g[2].reversed.split("\n").map(_.prepends(f)).join("\n"),trailNewlines:g[1],
...
$static($property($constant(a[c])));return a}));this.run=this.$(this.run)},run:$interlocked(function(a,b,c){var d=3===arguments.length?c:_.identity,e=this.runConfig=_.extend({silent:!0,verbose:!1,timeout:2E3,testStarted:function(a){},testComplete:function(a){}},b),f=_.map(e.suites||[],this.$(function(a){return this.testSuite(a.name,a.tests,e.context)}));(!1===e.codebase?_.cps.constant([]):this.$(this.collectPrototypeTests))(this.$(function(b){var c=!1===e.codebase?[]:this.collectTests();b=_.flatten(_.pluck(c.concat(f).concat(b),
"tests"));b=_.filter(b,e.shouldRun||_.constant(!0));this.runningTests=_.map(b,function(a,b){return _.extend(a,{indent:e.indent,index:b})});_.cps.each(b,this.$(this.runTest),this.$(function(){_.assert(!0!==e.done);e.done=!0;this.printLog(e);this.failedTests=_.filter(this.runningTests,_.property("failed"));this.failed=0<this.failedTests.length;d(!this.failed);a()}))}))}),defineAssertions:function(a){_.each(a,function(a,c){this.defineAssertion(c,a)},this)},runTest:function(a,b,c){var d=this,e=this.runConfig;
this.currentTest=a;e.testStarted(a);a.verbose=e.verbose;a.timeout=e.timeout;a.run(function(){e.testComplete(a);delete d.currentTest;c()})},collectTests:function(){return _.map(_.tests,this.$(function(a,b){return this.testSuite(b,a)}))},collectPrototypeTests:function(a){_.cps.map(this.prototypeTests,this.$(function(a,c){a.readPrototypeMeta(this.$(function(d){c(this.testSuite(d.name,a.tests))}))}),a)},testSuite:function(a,b,c){return{name:a||"",tests:_(_.pairs("function"===typeof b&&_.object([[a,b]])||
b)).map(function(b){return new Test({name:b[0],routine:b[1],suite:a,context:c})})}},defineAssertion:function(a,b){var c=this;_.deleteKeyword(a);_.defineKeyword(a,Tags.modifySubject(b,function(d){return _.withSameArgs(d,function(){return c.currentTest?c.currentTest.runAssertion(a,b,d,arguments):d.apply(c,arguments)})}))},printLog:function(a){if(!a.supressLog){var b=_.filter(this.runningTests,function(b){return b.failed||!a.silent&&b.hasLog}),c=_.filter(this.runningTests,_.property("failed"));_.invoke(a.verbose?
...
this.assertionStack([]);this.failureLocations={};var c=Tags.unwrap(this.routine),d=function(a){var d=function(){b.done=!0;a()};try{_.noArgs(c)?(c.call(b.context),d()):b.tryCatch(c,d)}catch(e){b.onException(e,a)}},e=function(a){b.assertionStack.when(_.isEmpty,a)},f=_.withTimeout.partial({maxTime:b.timeout,expired:function(a){b.failed=!0;log.error("TIMEOUT EXPIRED");a()}}),g=log.withWriteBackend.partial(_.extendWith({indent:b.depth+(b.indent||0)},function(a){b.logCalls.push(a)})),h=_.withUncaughtExceptionHandler.partial(b.$(b.onUnhandledException));
g(function(a){h(function(c){f(function(a){_.cps.sequence(d,e,a)()},function(){b.routine.$shouldFail&&(b.failed=!b.failed);(b.hasLog=0<b.logCalls.length)||(b.failed?log.red("FAIL"):b.verbose&&log.green("PASS"));c();a()})})},a)},printLog:function(){var a=this.suite&&this.suite!==this.name&&(this.suite||"").quote("[]")||"";log.write(log.color.blue,"\n"+log.boldLine,"\n"+_.nonempty([a,this.name]).join(" "),(this.index+" of "+Testosterone.runningTests.length).quote("()")+(this.failed?" FAILED":"")+":",
"\n");this.evalLogCalls()},evalLogCalls:function(){_.each(this.logCalls,log.writeBackend().arity1)}});Platform.NodeJS&&(module.exports=Testosterone);_.measure=function(a,b){if(b){var c=_.now();a(function(){b(_.now()-c)})}else return c=_.now(),a(),_.now()-c};_.perfTest=function(a,b){var c=_.isFunction(a)?{test:a}:a,d={};_.cps.each(c,function(a,b,c){var h=[],k=function(){for(var c=0;500>c;c++)h.push(a());console.log(b,h)};k();_.delay(function(){d[b]=_.measure(k)/500;c()},100)},function(){b(d)})};
...
a)?a&&a.stack:_.isTypeOf(Test,a)?a.suite+a.name:a)||"").hash},print:function(a,c){return _.isTypeOf(Error,a)?this.printError(a):_.isTypeOf(Test,a)?this.printFailedTest(a):this.printUnknownStuff(a,c)},printUnknownStuff:function(b,c){return c?b:a("<span>").text(log.impl.stringify(b))},printFailedTest:function(b){var c=a('<div class="test-log" style="margin-top: 13px;">');log.withWriteBackend(function(b){c.append(a("<pre>").css({color:b.color.css}).html(_.escape(b.indentedText)+(b.codeLocation&&' <span class="location">'+
