...
$jscomp.iteratorPrototype=function(a){$jscomp.initSymbolIterator();a={next:a};a[$jscomp.global.Symbol.iterator]=function(){return this};return a};$jscomp.makeIterator=function(a){$jscomp.initSymbolIterator();var b=a[Symbol.iterator];return b?b.call(a):$jscomp.arrayIterator(a)};$uselessFile="useless.micro.js";"undefined"!==typeof require&&(_=require("underscore"),$include=require);
_=function(){_.mixin({zipWith:function(a,b){return _.reduce(_.rest(a),function(a,d){return _.times(Math.max(a&&a.length||0,d&&d.length||0),function(e){return b(a&&a[e],d&&d[e])})},_.first(a))}});if("a1 b2 c3"!==_.zipWith([["a","b","c"],[1,2,3]],function(a,b){return a+b}).join(" "))throw Error("_.zipWith broken");return _}();_.tests={};_.deferTest=_.withTest=function(a,b,c){c()};
...
$overrideUnderscore=function(a,b){return _[a]=b(_[a])};$platform.NodeJS&&($global.alert=function(a){var b=$global.log&&_.partial(log.warn,log.config({stackOffset:2}))||console.log;b.apply(b,["ALERT:"].concat(_.asArray(arguments)))});$global.alert2=function(a){alert(_.map(arguments,_.stringify).join(", "));return arguments[0]};$global.log=function(){console.log.apply(console,arguments)};_.extend(_,{asArray:function(a){return void 0!==a.length?[].slice.call(a,0):[a]}});
...
$uselessFile="panic.js";_.hasAsserts=!0;
...
a()})},$macroTags:{log:function(b,c,d){var e=(_.isBoolean(c.$log)?void 0:c.$log)||(c.$verbose?"{{$proto}}":""),f={},g=_.find2(a,function(a){return log.color(c["$"+a]&&a)||!1}),h=e&&_.template(e,{interpolate:/\{\{(.+?)\}\}/g});$untag(b.$meta)(function(a){f=a});return $prototype.impl.modifyMember(c,function(a,b){return function(){var e=this,m=_.asArray(arguments),p=h&&h.call(this,_.extend({$proto:f.name},_.map2(this,_.stringifyOneLine.arity1)))||this.desc||"",q=_.map(m,_.stringifyOneLine.arity1).join(", ").quote("()");
...
function(c,f,g){return a('<div class="line">').append(_.escape(b.indentation)).append(_.map(c,function(b){return a("<span>").attr("style",b.config.color&&b.config.color.css||"").text(b.text)})).append(f===g.lastIndex?[b.where&&this.printLocation(b.where),b.trailNewlines.replace(/\n/g,"<br>")]:[])},this)))}),function(a){b.evalLogCalls();a()});return[a('<div class="panic-alert-error-message" style="font-weight: bold;">').text(b.name).append('<span style="float:right; opacity: 0.25;">test failed</span>'),
...
[]),function(b){return a("<pre>").text(log.impl.stringify(b))})),a('<ul class="callstack">').append(_.map(c,this.$(function(b){var c=a('<li class="callstack-entry">').toggleClass("third-party",b.thirdParty||!1).toggleClass("hide",b.hide||!1).toggleClass("native",b["native"]||!1).append([a('<span class="file">').text(_.nonempty([b.index?"(index)":b.fileShort,b.line]).join(":")),a('<span class="callee">').text(b.calleeShort),a('<span class="src i-am-busy">').click(this.$(function(f){var g=a(f.delegateTarget);
g.waitUntil(SourceFiles.read.partial((b.remote?"api/source/":"")+b.file),this.$(function(f){c.is(".full")?(c.removeClass("full"),c.transitionend(function(){c.is(".full")||b.sourceReady(g.$(a.fn.text))})):(c.addClass("full"),g.html(_.map(f.split("\n"),function(b){return a('<div class="line">').text(b)})),f=g.find(".line").eq(b.line-1).addClass("hili"),f.length&&(f=f.offset().top-g.offset().top,g.scrollTop(f-100)),_.delay(this.$(function(){var a=g.outerBBox().bottom+242-this.modalBody.outerBBox().bottom;
...
c?a(this).scrollLeft():a(this).scrollTop(),g=c?a(this).width():a(this).height(),h=(c?this.scrollWidth:this.scrollHeight)-1;d.css({opacity:0<b?1:0});e.css({opacity:b+g<h?1:0})}).scroll();return this}})})(jQuery);
