...
k=b._onAfter,l=b._intercept,b=d||this,n,m=void 0;if(f.length){n=0;for(m=f.length;n<m;n++)f[n].apply(b,arguments);f.removeAll()}n=0;for(m=h.length;n<m;n++)h[n].apply(b,arguments);f=(l.length?_.cps.compose([a].concat(l)):a).apply(b,arguments);if(k.length||g.length){h=_.asArray(arguments).concat(f);n=0;for(m=k.length;n<m;n++)k[n].apply(b,h);if(g.length){n=0;for(m=g.length;n<m;n++)g[n].apply(b,h);g.removeAll()}}return f}))})})})();
...
a('<span class="file">').text(b.fileName),a('<span class="line">').text(b.line)])},printFailedTest:function(b){var c=a('<pre class="test-log" style="margin-top: 13px;">');log.withWriteBackend(this.$(function(b){_.isTypeOf(Error,b.args.first)&&console.log(b.args.first);c.append(_.isTypeOf(Error,b.args.first)?a("<div>").append([_.escape(b.indentation),a('<div class="panic-alert-error inline-exception all-stack-entries">').append(this.printError(b.args.first))]):a('<div class="log-entry">').append(_.map(b.lines,
