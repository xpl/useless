...
c=log.cleanArgs(b),d=_.extend({indent:0},a,log.readConfig(b)),e=(log.impl.writeBackend.indent||0)+d.indent,f=log.impl.stringifyArguments(c,d),e=_.times(e,_.constant("\t")).join(""),f=f.reversed.match(/(\n*)([^]*)/),d=d.location&&log.impl.location(d.where||$callStack[3+(d.stackOffset||0)])||"",b={color:log.readColor(b),indentedText:f[2].reversed.split("\n").map(_.prepends(e)).join("\n"),trailNewlines:f[1],codeLocation:d};log.impl.writeBackend(b);return c[0]})},defaultWriteBackend:function(a){var b=
