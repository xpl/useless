...
_.defineGlobalProperty("alert2",function(a){alert(_.map(arguments,_.stringify).join(", "));return arguments[0]});_.global().log=function(){console.log.call(console.log,arguments)};_.extend(_,{asArray:function(a){return void 0!==a.length?[].slice.call(a,0):[a]}});
...
_.camelCaseToDashes=function(a){return a.replace(/[a-z][A-Z]/g,function(a){return a[0]+"-"+a[1].lowercase})};_.camelCaseToLoDashes=function(a){return a.replace(/[a-z][A-Z]/g,function(a){return a[0]+"_"+a[1].lowercase})};_.dashesToCamelCase=function(a){return a.replace(/(-.)/g,function(a){return a[1].uppercase})};_.loDashesToCamelCase=function(a){return a.replace(/(_.)/g,function(a){return a[1].uppercase})};
...
_.interlocked=function(a){var b=new Lock;return _.extendWith({wait:b.$(b.wait)},_.argumentPrependingWrapper(Tags.unwrap(a),function(a){b.acquire(function(){a(b.$(b.release))})}))};_.defineKeyword("scope",function(a){var b=void 0;return _.argumentPrependingWrapper(Tags.unwrap(a),function(a){var d={when:void 0};(b=b||[]).push(d);a(function(a){if(d.when)throw Error("$scope: release called twice");for(d.when=a;b&&b.last&&b.last.when;)a=b.last.when,(b=_.initial(b)).isEmpty&&(b=void 0),a()})})});
Platform.NodeJS&&(module.exports=_);_.defineKeyword("component",function(a){return $extends(Component,a)});_("extendable trigger triggerOnce barrier observable bindable memoize interlocked memoizeCPS debounce throttle overrideThis listener postpones reference".split(" ")).each(_.defineTagKeyword);_.defineTagKeyword("observableProperty",_.flip);_.defineKeyword("observableRef",function(a){return $observableProperty($reference(a))});
$prototype.macro("$depends",function(a,b,c){a.$depends=$builtin($const(_.coerceToArray(b)));return a});$prototype.macroTag("extendable",function(a,b,c){a[c]=$builtin($const(b));return a});
...
c)).insertAfter(this.el.find(".panic-modal-title"));this.toggleVisibility(!0);this.layout()},hash:function(a){return((_.isTypeOf(Error,a)?a&&a.stack:_.isTypeOf(Test,a)?a.suite+a.name:a)||"").hash},print:function(a,c){return _.isTypeOf(Error,a)?this.printError(a):_.isTypeOf(Test,a)?this.printFailedTest(a):this.printUnknownStuff(a,c)},printUnknownStuff:function(b,c){return c?b:a("<span>").text(log.impl.stringify(b))},printLocation:function(b){return a('<span class="location">').append([a('<span class="callee">').text(b.calleeShort),
a('<span class="file">').text(b.fileName),a('<span class="line">').text(b.line)])},printFailedTest:function(b){var c=a('<pre class="test-log" style="margin-top: 13px;">');log.withWriteBackend(this.$(function(b){_.isTypeOf(Error,b.args.first)&&console.log(b.args.first);c.append(_.isTypeOf(Error,b.args.first)?a("<div>").css({color:b.color&&b.color.css||""}).append([_.escape(b.indentation),a('<div class="panic-alert-error inline-exception all-stack-entries">').append(this.printError(b.args.first))]):
a('<div class="log-entry">').append(_.map(b.lines,function(c,f,g){return a('<div class="line">').append(_.escape(b.indentation)).append(_.map(c,function(b){return a("<span>").css({color:b.config.color&&b.config.color.css||""}).text(b.text)})).append(f===g.lastIndex?[b.where&&this.printLocation(b.where),b.trailNewlines.replace(/\n/g,"<br>")]:[])},this)))}),function(a){b.evalLogCalls();a()});return[a('<div class="panic-alert-error-message" style="font-weight: bold;">').text(b.name).append('<span style="float:right; opacity: 0.25;">test failed</span>'),
...
[]),function(b){return a("<pre>").text(log.impl.stringify(b))})),a('<ul class="callstack">').append(_.map(c,this.$(function(b){var c=a('<li class="callstack-entry">').toggleClass("third-party",b.thirdParty).toggleClass("native",b["native"]).append([a('<span class="file">').text(_.nonempty([b.index?"(index)":b.fileShort,b.line]).join(":")),a('<span class="callee">').text(b.calleeShort),a('<span class="src i-am-busy">').click(this.$(function(f){var g=a(f.delegateTarget);g.waitUntil(_.readSource.partial((b.remote?
...
(function(a){LogOverlay=$singleton(Component,{$defaults:{opaque:!1,init:!1},init:function(){log.withWriteBackend(this.write,function(){});a(document).keydown(this.$(function(a){192===a.keyCode?this.toggle():27===a.keyCode&&this.el.empty()}))},el:$memoized($property(function(){return a('<div class="useless-log-overlay" style="display: none;">').appendTo(document.body)})),toggle:function(a){this.el.toggle(a)},visible:$property(function(){return this.el.is(":visible")}),write:function(b){this.toggle(!0);
b.config.clear&&this.el.empty();this.el.append(a('<div class="ulo-line">').css("color",b.color&&b.color.css||"").append(a('<span class="ulo-line-text">').text(b.indentedText+" ")).append(a('<span class="ulo-line-where">').text(b.codeLocation+" ")).append(a('<span class="ulo-line-trail">').text(b.trailNewlines)));this.opaque||log.impl.defaultWriteBackend(b)}})})(jQuery);
