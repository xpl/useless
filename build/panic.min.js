...
_.defineKeyword("interlocked",function(a){var b=new Lock;return _.extendWith({wait:b.$(b.wait)},_.prependsArguments(Tags.unwrap(a),function(a){b.acquire(function(){a(b.$(b.release))})}))});_.defineKeyword("scope",function(a){var b=void 0;return _.prependsArguments(Tags.unwrap(a),function(a){var d={when:void 0};(b=b||[]).push(d);a(function(a){if(d.when)throw Error("$scope: release called twice");for(d.when=a;b&&b.last&&b.last.when;)a=b.last.when,(b=_.initial(b)).isEmpty&&(b=void 0),a()})})});
...
{parent_:void 0,children_:[]},_.omit(_.omit(c,"init","attachTo","attach"),function(a,b){return Component.isStreamDefinition(d[b])},this));var e=[];_.each(d,function(a,b){if(void 0!==a){if(a.$observableProperty){var d=this[b];defaultValue=b in c?c[b]:d;var k=this[b+"Change"]=_.observable();k.context=this;k.postpones=a.$postpones;if(_.isPrototypeInstance(d)){var m=d.constructor;k.beforeWrite=function(a){return m.isTypeOf(a)?a:new m(a)}}a.$reference&&(k.trackReference=!0);_.defineProperty(this,b,{get:function(){return k.value},
...
e-b&&a.apply(this,arguments);b=e})}return this.dblclick(a)},nodoubletapzoom:function(){return a(this).bind("touchstart",function(b){var d=b.timeStamp,e=a(this).data("lastTouch")||d,e=d-e,g=b.originalEvent.touches.length;a(this).data("lastTouch",d);!e||500<e||1<g||(b.preventDefault(),a(b.target).trigger("click"))})}})}(jQuery);$uselessFile="panic.js";_.defineTagKeyword("required");_.defineTagKeyword("atom");_.defineKeyword("any",_.identity);
...
run:$interlocked(function(a,b,c){var d=3===arguments.length?c:_.identity,e=this.runConfig=_.extend({silent:!0,verbose:!1,timeout:2E3,testStarted:function(a){},testComplete:function(a){}},b),g=_.map(e.suites||[],this.$(function(a){return this.testSuite(a.name,a.tests,e.context)}));(!1===e.codebase?_.cps.constant([]):this.$(this.collectPrototypeTests))(this.$(function(b){var c=!1===e.codebase?[]:this.collectTests();b=_.flatten(_.pluck(c.concat(g).concat(b),"tests"));var k=_.filter(b,e.shouldRun||_.constant(!0));
this.runningTests=_.map(k,function(a,b){return _.extend(a,{indent:e.indent,index:b})});_.withUncaughtExceptionHandler(this.$(this.onException),this.$(function(b){_.cps.each(k,this.$(this.runTest),this.$(function(){_.assert(!0!==e.done);e.done=!0;this.printLog(e);this.failedTests=_.filter(this.runningTests,_.property("failed"));this.failed=0<this.failedTests.length;d(!this.failed);b();a()}))}))}))}),onException:function(a){if(this.currentAssertion)this.currentAssertion.onException(a);else throw a;
},defineAssertions:function(a){_.each(a,function(a,c){this.defineAssertion(c,a)},this)},runTest:function(a,b,c){var d=this,e=this.runConfig;e.testStarted(a);a.verbose=e.verbose;a.timeout=e.timeout;a.startTime=Date.now();a.run(function(){e.testComplete(a);a.time=Date.now()-a.startTime;d.currentAssertion=void 0;c()})},collectTests:function(){return _.map(_.tests,this.$(function(a,b){return this.testSuite(b,a)}))},collectPrototypeTests:function(a){_.cps.map(this.prototypeTests,this.$(function(a,c){a.readPrototypeMeta(this.$(function(d){c(this.testSuite(d.name,
...
Test=$prototype({constructor:function(a){_.defaults(this,a,{name:"youre so dumb you cannot even think of a name?",failed:!1,routine:void 0,verbose:!1,depth:1,indent:0,failedAssertions:[],context:this,complete:_.barrier()});this.babyAssertion=$interlocked(this.babyAssertion)},finalize:function(){this.babyAssertion.wait(this.$(function(){this.canFail&&this.failedAssertions.length&&(this.failed=!0);this.complete(!0)}))},babyAssertion:function(a,b,c,d,e,g){var f=this,h=new Test({mother:this,name:b,shouldFail:c.$shouldFail||
this.shouldFail,depth:this.depth+1,location:g,context:this.context,timeout:this.timeout/2,verbose:this.verbose,silent:this.silent,routine:Tags.modifySubject(c,function(a){return function(b){if($async.is(e[0]))_.cps.apply(a,f.context,e,function(a,c){c&&c();b()});else try{a.apply(f.context,e),b()}catch(c){h.onException(c)}}})}),k=function(){h.failed&&f.canFail&&f.failedAssertions.push(h);Testosterone.currentAssertion=f;a()};h.run(function(){h.failed||h.verbose&&h.logCalls.notEmpty?h.location.sourceReady(function(a){log.red(a,
log.config({location:h.location,where:h.location}));h.evalLogCalls();k()}):k()})},canFail:$property(function(){return!this.failed&&!this.shouldFail}),fail:function(){this.failed=!0;this.finalize()},onException:function(a){if(this.canFail||this.verbose){if(_.isAssertionError(a)){if("notMatching"in a){var b=_.coerceToArray(a.notMatching);a.asColumns?log.orange(log.columns(_.map(b,function(a){return["\u2022 "+_.keys(a)[0],_.stringify(_.values(a)[0])]})).join("\n")):_.each(b,function(a,b){log.orange("\u2022",
a)})}}else 1<this.depth&&log.newline(),log.write(a);log.newline()}this.canFail?this.fail():this.finalize()},run:function(a){var b=Testosterone.currentAssertion=this,c=Tags.unwrap(this.routine);this.shouldFail=$shouldFail.is(this.routine);this.hasLog=this.failed=!1;this.logCalls=[];this.failureLocations={};_.withTimeout({maxTime:b.timeout,expired:function(){b.canFail&&(log.error("TIMEOUT EXPIRED"),b.fail())}},function(a){b.complete(a.arity0)});log.withWriteBackend(_.extendWith({indent:b.depth+(b.indent||
0)},function(a){b.logCalls.push(a)}),function(d){b.complete(d.arity0);a&&b.complete(a);0<c.length?c.call(b.context,b.$(b.finalize)):(c.call(b.context),b.finalize())})},printLog:function(){var a=this.suite&&this.suite!==this.name&&(this.suite||"").quote("[]")||"";log.write(log.color.blue,"\n"+log.boldLine,"\n"+_.nonempty([a,this.name]).join(" "),(this.index+" of "+Testosterone.runningTests.length).quote("()")+(this.failed?" FAILED":"")+":","\n");this.evalLogCalls()},evalLogCalls:function(){_.each(this.logCalls,
log.writeBackend().arity1)}});Platform.NodeJS&&(module.exports=Testosterone);_.measure=function(a,b){if(b){var c=_.now();a(function(){b(_.now()-c)})}else return c=_.now(),a(),_.now()-c};_.perfTest=function(a,b){var c=_.isFunction(a)?{test:a}:a,d={};_.cps.each(c,function(a,b,c){var h=[],k=function(){for(var c=0;500>c;c++)h.push(a());console.log(b,h)};k();_.delay(function(){d[b]=_.measure(k)/500;c()},100)},function(){b(d)})};
...
a)?a&&a.stack:_.isTypeOf(Test,a)?a.suite+a.name:a)||"").hash},print:function(a,c){return _.isTypeOf(Error,a)?this.printError(a):_.isTypeOf(Test,a)?this.printFailedTest(a):this.printUnknownStuff(a,c)},printUnknownStuff:function(b,c){return c?b:a("<span>").text(log.impl.stringify(b))},printFailedTest:function(b){var c=a('<pre class="test-log" style="margin-top: 13px;">');log.withWriteBackend(function(b){c.append(a("<div>").css({color:b.color.css}).html(_.escape(b.indentedText)+(b.codeLocation&&' <span class="location">'+
...
b.color.css).append(a('<span class="ulo-line-text">').text(b.indentedText+" ")).append(a('<span class="ulo-line-where">').text(b.codeLocation+" ")).append(a('<span class="ulo-line-trail">').text(b.trailNewlines)));this.opaque||log.impl.defaultWriteBackend(arguments)}})})(jQuery);
