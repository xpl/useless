...
name:b,shouldFail:c.$shouldFail||this.shouldFail,depth:this.depth+1,location:f,context:this.context,timeout:this.timeout/2,verbose:this.verbose,silent:this.silent,routine:Tags.modify(c,function(a){return function(b){if($async.is(e[0])||$async.is(c))_.cps.apply(a,g.context,e,function(a,c){c&&c();b()});else try{a.apply(g.context,e),b()}catch(d){h.onException(d)}}})}),k=function(){h.failed&&g.canFail&&g.failedAssertions.push(h);a()};h.run(function(){Testosterone.currentAssertion=g;h.failed||h.verbose&&
