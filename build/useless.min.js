...
0<e.precision?_.toFixed(a,e.precision):a+"";var h=_.isArray(a),g=e.pretty||!1;if(a.toJSON)return _.quoteWith('"',a.toJSON());if(!e.pure&&(d>(e.maxDepth||5)||h&&a.length>(e.maxArrayLength||30)))return h?"<array["+a.length+"]>":"<object>";var k=b.concat([a]);c.push(a);b=_.pairs(a);var l=!g||2>b.length,m=f+1,n=l?"":"\t".repeats(m);if(g&&!h){var p=_.reduce(_.map(_.keys(a),_.count),_.largest,0);b=_.map(b,function(a){return[a[0],a[1]," ".repeats(p-a[0].length)]})}return _.quoteWith(h?l?"[]":"[\n  ]":l?
...
Sort={Ascending:1,Descending:-1,strings:function(a,b){a=$.trim(a).toLowerCase();b=$.trim(b).toLowerCase();return 0==a.length&&0<b.length?1:0<a.length&&0==b.length?-1:a==b?0:a<b?-1:1},numbers:function(a,b){return isNaN(a)&&isNaN(b)?0:isNaN(a)?-1:isNaN(b)?1:a<b?-1:a>b?1:0},generic:function(a,b){return a||b?a?b?a<b?-1:a>b?1:0:1:-1:0},inverse:function(a){return function(b,c){return-a(b,c)}},field:function(a,b,c){return function(d,e){return b(d[a],e[a])*c}}};_.hasLog=!0;
...
"$",function(){return R.expr(a.$$)});return a})}});_.defineKeyword("callStack",function(){return CallStack.fromRawString(CallStack.currentAsRawString).offset(Platform.NodeJS?1:0)});_.defineKeyword("currentFile",function(){return(CallStack.rawStringToArray(CallStack.currentAsRawString)[Platform.NodeJS?3:1]||{file:""}).file});_.defineKeyword("uselessPath",_.memoize(function(){return _.initial($currentFile.split("/"),Platform.NodeJS?2:1).join("/")+"/"}));
...
CallStack=$extends(Array,{enableSetTimeoutHook:$static(function(){var a=$global.setTimeout;$global.setTimeout=function(b,c){var d=CallStack.current;return a(function(){_.withUncaughtExceptionHandler(function(a){throw _.extend(a,{parsedStack:CallStack.fromParsedArray(_.initial(CallStack.fromError(a).asArray,3).concat(d.asArray))});},function(a){b();a()})},c)}}),current:$static($property(function(){return CallStack.fromRawString(CallStack.currentAsRawString).offset(1)})),fromError:$static(function(a){$global.xxx=
a;return a.parsedStack?CallStack.fromParsedArray(_.map(a.parsedStack,function(a){return _.extend(a,{sourceReady:_.constant(a.source)})})):CallStack.fromRawString(a.stack)}),locationEquals:$static(function(a,b){return a.file===b.file&&a.line===b.line&&a.column===b.column}),safeLocation:function(a){return this[a]||{callee:"",calleeShort:"",file:"",fileName:"",fileShort:"",thirdParty:!1,source:"??? WRONG LOCATION ???",sourceReady:_.cps.constant("??? WRONG LOCATION ???")}},clean:$property(function(){return this.reject(_.property("thirdParty"))}),
asArray:$property(function(){return _.asArray(this)}),offset:function(a){return CallStack.fromParsedArray(_.rest(this,a))},filter:function(a){return CallStack.fromParsedArray(_.filter(this,a))},reject:function(a){return CallStack.fromParsedArray(_.reject(this,a))},reversed:$property(function(){return CallStack.fromParsedArray(_.reversed(this))}),sourcesReady:function(a){return _.allTriggered(_.pluck(this,"sourceReady"),a)},constructor:function(a){Array.prototype.constructor.call(this);for(var b=0,
c=a.length;b<c;b++)this.push(a[b])},fromParsedArray:$static(function(a){return new CallStack(a)}),currentAsRawString:$static($property(function(){var a="browser"===_.platform().engine?3:2;return _.rest((Error().stack||"").split("\n"),a).join("\n")})),shortenPath:$static(function(a){return a.replace($uselessPath,"").replace($sourcePath,"")}),isThirdParty:$static(_.bindable(function(a){var b=a.replace($sourcePath,"");return Platform.NodeJS&&"/"!==a[0]||0<=b.indexOf("/node_modules/")||0<=a.indexOf("/node_modules/")&&
!b||0<=b.indexOf("underscore")||0<=b.indexOf("jquery")})),fromRawString:$static(_.sequence(function(a){return CallStack.rawStringToArray(a)},function(a){return _.map(a,function(a){return _.extend(a,{calleeShort:_.last(a.callee.split(".")),fileName:_.last(a.file.split("/")),fileShort:CallStack.shortenPath(a.file),thirdParty:CallStack.isThirdParty(a.file)})})},function(a){return _.map(a,function(a){a.source="";a.sourceReady=_.barrier();_.readSourceLine(a.file,a.line-1,function(c){a.source=c;a.sourceReady(c)});
return a})},function(a){return CallStack.fromParsedArray(a)})),rawStringToArray:$static(function(a){a=_.rest((a||"").split("\n"),"browser"===_.platform().engine?1:0);return _.map(a,function(a){a=a.trimmed;var c,d=[],e=a.match(/at (.+) \((.+)\)/);e?(c=e[1],d=_.rest(e[2].match(/(.*):(.+):(.+)/)||[])):(e=a.match(/at (.+)/))&&e[1]&&(d=_.rest(e[1].match(/(.*):(.+):(.+)/)||[]));return{beforeParse:a,callee:c||"",file:d[0]||"",line:(d[1]||"").integerValue,column:(d[2]||"").integerValue}})})});
$prototype.macro(function(a,b){var c=CallStack.currentAsRawString;a.$meta||(a.$meta=$static(_.cps.memoize(function(a){_.cps.find(CallStack.fromRawString(c).reversed,function(a,b){a.sourceReady(function(c){c=(c||"").match(/([A-z]+)\s*=\s*\$(prototype|singleton|component|extends|trait|aspect)/);b(c&&{name:c[1],type:c[2],file:a.fileShort}||!1)})},function(b){a(b||{})})})));return a});_.measure=function(a,b){if(b){var c=_.now();a(function(){b(_.now()-c)})}else return c=_.now(),a(),_.now()-c};
_.perfTest=function(a,b){var c=_.isFunction(a)?{test:a}:a,d={};_.cps.each(c,function(a,b,c){var h=[],k=function(){for(var c=0;500>c;c++)h.push(a());console.log(b,h)};k();_.delay(function(){d[b]=_.measure(k)/500;c()},100)},function(){b(d)})};_.defineTagKeyword("shouldFail");_.defineTagKeyword("async");_.defineTagKeyword("assertion");
...
"\n");this.evalLogCalls()},evalLogCalls:function(){_.each(this.logCalls,function(a){log.impl.writeBackend(a)})}});Platform.NodeJS&&(module.exports=Testosterone);
