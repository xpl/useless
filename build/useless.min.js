...
arguments.callee,f=b._onceBefore,g=b._onceAfter,h=b._onBefore,k=b._onAfter,l=b._intercept,b=d||this,n,m=void 0;if(f.length){n=0;for(m=f.length;n<m;n++)f[n].apply(b,arguments);f.removeAll()}n=0;for(m=h.length;n<m;n++)h[n].apply(b,arguments);f=(l.length?_.cps.compose([a].concat(l)):a).apply(b,arguments);if(k.length||g.length){h=_.asArray(arguments).concat(f);n=0;for(m=k.length;n<m;n++)k[n].apply(b,h);if(g.length)for(k=g.copy,g.removeAll(),n=0,m=k.length;n<m;n++)k[n].apply(b,h)}return f}))})})})();
...
c){a.callee=(a.callee||"<anonymous>")+" \u2192 "+(c.callee||"<anonymous>");a.calleeShort=(a.calleeShort||"<anonymous>")+" \u2192 "+(c.calleeShort||"<anonymous>");return a},_.clone(a[0]))}))}),clean:$property(function(){var a=this.mergeDuplicateLines.reject(function(a){return a.thirdParty||(a.source||"").contains("// @hide")});return 0===a.length?this:a}),asArray:$property(function(){return _.asArray(this)}),offset:function(a){return a&&CallStack.fromParsedArray(_.rest(this,a))||this},initial:function(a){return a&&
CallStack.fromParsedArray(_.initial(this,a))||this},concat:function(a){return CallStack.fromParsedArray(this.asArray.concat(a.asArray))},filter:function(a){return CallStack.fromParsedArray(_.filter(this,a))},reject:function(a){return CallStack.fromParsedArray(_.reject(this,a))},reversed:$property(function(){return CallStack.fromParsedArray(_.reversed(this))}),sourcesReady:function(a){return _.allTriggered(_.pluck(this,"sourceReady"),a)},constructor:function(a){Array.prototype.constructor.call(this);
_.each(a,function(a){a.sourceReady||(a.sourceReady=_.barrier(),SourceFiles.line((a.remote?"api/source/":"")+a.file,a.line-1,function(c){a.sourceReady(a.source=c)}));this.push(a)},this)},fromParsedArray:$static(function(a){return new CallStack(a)}),currentAsRawString:$static($property(function(){var a=$platform.Browser?3:2;return _.rest((Error().stack||"").split("\n"),a).join("\n")})),shortenPath:$static(function(a){var b=a.replace($uselessPath,"").replace($sourcePath,"");return b!==a?b:a.split("/").last}),
isThirdParty:$static(_.bindable(function(a){var b=a.replace($sourcePath,"");return $platform.NodeJS&&"/"!==a[0]||0<=b.indexOf("/node_modules/")||0<=a.indexOf("/node_modules/")&&!b||0<=b.indexOf("underscore")||0<=b.indexOf("jquery")})),fromRawString:$static(_.sequence(function(a){return CallStack.rawStringToArray(a)},function(a){return _.map(a,function(a){return _.extend(a,{calleeShort:_.last(a.callee.split(".")),fileName:_.last(a.file.split("/")),fileShort:CallStack.shortenPath(a.file),thirdParty:CallStack.isThirdParty(a.file)&&
!a.index})})},function(a){return CallStack.fromParsedArray(a)})),rawStringToArray:$static(function(a){a=(a||"").split("\n");return _.filter2(a,function(a){a=a.trimmed;var c,d=[],e=!1,d=d=void 0;if((d=a.match(/at (.+) \((.+)\)/))||(d=a.match(/(.*)@(.*)/)))c=d[1],e="native"===d[2],d=_.rest(d[2].match(/(.*):(.+):(.+)/)||[]);else if(d=a.match(/^(at\s+)*(.+):([0-9]+):([0-9]+)/))d=_.rest(d,2);else return!1;return 0<=(c||"").indexOf("__supressErrorReporting")?!1:{beforeParse:a,callee:c||"",index:$platform.Browser&&
d[0]===window.location.href,"native":e,file:d[0]||"",line:(d[1]||"").integerValue,column:(d[2]||"").integerValue}})})});$prototype.impl.findMeta=function(a){return function(b){_.cps.find(CallStack.fromRawString(a).reversed,function(a,b){a.sourceReady(function(e){e=(e||"").match(/([A-z]+)\s*=\s*\$(prototype|singleton|component|extends|trait|aspect)/);b(e&&{name:e[1],type:e[2],file:a.fileShort}||!1)})},function(a){b(a||{})})}};
...
c(g)})})))),d=_.pluck(d,"text").join(""),h=c.where||log.impl.walkStack($callStack)||{},k="\t".repeats(c.indent);a({lines:e,config:c,color:c.color,when:Date.now(),args:_.reject(b,_.isTypeOf.$(log.Config)),indentation:k,indentedText:e.map(_.seq(_.pluck.tails2("text"),_.joinsWith(""),_.prepends(k))).join("\n"),text:d,codeLocation:c.location&&log.impl.location(h)||"",trailNewlines:f||"",where:c.location&&h||void 0});return _.find(b,_.not(_.isTypeOf.$(log.Config)))})),walkStack:function(a){return _.find(a.clean.offset(2),
...
(function(){$mixin(Promise,{shouldBe:function(b){return this.then(function(c){if(b!==c)throw new a;})},shouldFail:$property(function(){return this.then(function(){throw new a;},function(){})})});var a=function(a){Error.call(this,a||"assertion failed")};$jscomp.inherits(a,Error);a.captureStackTrace=Error.captureStackTrace;var b=Promise;$global.AndrogeneProcessContext=$prototype({current:void 0,constructor:function(){this.eventLog=[];this.callStack=$callStack;this.state="pending";void 0!==(this.parent=
AndrogeneProcessContext.current)&&(this.parent.eventLog.push(this),this.env=this.parent.env)},root:$property(function(){return this.parent&&this.parent.root||this}),push:$static(function(a){var b=AndrogeneProcessContext.current;AndrogeneProcessContext.current=a;var c=Promise;Promise=AndrogenePromise;var g=function(){a.eventLog.push([log.config({where:$callStack.safeLocation(5)})].concat(_.initial(arguments)))};log.impl.write.intercept(g);return function(){AndrogeneProcessContext.current=b;Promise=
c;log.impl.write.off(g)}}),within:function(a){var b=this;return function(){var c=AndrogeneProcessContext.push(b);try{var g=a.apply(this,arguments);c();return g}catch(h){throw c(),h;}}},where:$property(function(){var a=this.callStack.reject(function(a){return a.source.contains("@hide")});return this.parent?a.last:a.first}),printWhere:function(a,b){a=a||0;var c=log.color[{fulfilled:"green",pending:"orange",rejected:"red","":"purple"}[this.state||""]];log.margin();for(var g=$jscomp.makeIterator(this.callStack.clean.reversed),
h=g.next();!h.done;h=g.next())h=h.value,log.write(c,log.config({indent:a,location:!0,where:h}),"\u00b7",h.source.trimmed);log.margin()},printEvents:function(a,b){a=a||0;b=b||new Set;for(var c=$jscomp.makeIterator(this.eventLog),g=c.next();!g.done;g=c.next())g=g.value,g instanceof AndrogeneProcessContext?g.eventLog.length&&g.printLog(a+1,b):g instanceof Error?b.has(g)||(b.add(g),log.boldRed(log.indent(a+1),g)):log.write.apply(null,[log.indent(a+1)].concat(g));log.margin()},printLog:function(a,b){this.printWhere(a,
b);this.printEvents(a,b)}});AndrogeneProcessContext.within=function(){return AndrogeneProcessContext.current&&AndrogeneProcessContext.current.within(fn)||fn};$global.AndrogenePromise=function(a){if(!0===AndrogenePromise.constructing)Promise.call(this,a);else{var b=new AndrogeneProcessContext,c,g;Promise.call(this,function(a,d){c=a;g=function(a){b.eventLog.push(a);d(a)}});this.processContext=b;try{b.within(a)(c,g)}catch(h){g(h)}AndrogenePromise.constructing=!0;Promise.prototype.then.call(this,function(a){b.state=
"fulfilled"},function(a){b.state="rejected"});delete AndrogenePromise.constructing}};$jscomp.inherits($global.AndrogenePromise,Promise);$global.AndrogenePromise.all=Promise.all;$global.AndrogenePromise.reject=Promise.reject;$global.AndrogenePromise.resolve=Promise.resolve;$global.AndrogenePromise.prototype.then=function(a,c){var f=this.processContext.within(b.prototype.then,2).apply(this,_.map(arguments,function(a){return a&&function(b){return f.processContext.within(a,-3)(b)}}));return f};$global.AndrogenePromise.prototype.disarmAndrogene=
function(){return new b(b.prototype.then.bind(this))};$global.AndrogenePromise.race=function(a){return b.race(a)};$global.AndrogenePromise.coerce=function(a){return a instanceof AndrogenePromise?a:a instanceof Function?new AndrogenePromise(function(b){b(a())}):AndrogenePromise.resolve(a)};var c=function(a){return function(){return AndrogenePromise.coerce(a)}};c(function(b,c){if(b!==c)throw new a;});c(function(a){for(var b=[],c=[],g=0,h=a.length;g<h;g++)b.push(new Promise(function(a){c.push(a.$(!0))}));
a.apply(this,c);return Promise.all(b)});c(function(){return __("foo").then(_.appends("bar")).shouldBe("foobr")});c(function(){return(new Promise(function(a){log.i("example log message #1");(new Promise(function(a){log.i("example log message #2");a()})).then(function(){log.i("example log message #3");a()})})).then(function(){log.i("example log message #4");some_undefined_function()})});c(function(){var a=(new Promise(function(a){log.i("A");a("A")})).then(function(a){log.i("B");return"B"});return(new Promise(function(a){log.i("C");
a("C")})).then(function(b){log.i("D");return a.then(function(a){log.i("E");return"E"})}).then(function(a){log.i("F");return"F"})});c(function(){return __("foo").then(_.appends("bar")).delay(500).log.then(function(){dasdsad()}).timeout(600)});c(function(){throw Error("yo");})})();
...
a.parse(e),k=h.name&&h.name.decapitalized||e;if(h=h.how&&_["on"+h.how.capitalized]||_.intercept){var l=b[k];l&&_.isFunction(l)&&h(b,k,c);var l=b.prototype,n=l[k];n&&_.isFunction(n)&&h(l,k,c)}}});b.aspectAdded&&b.aspectAdded(d);return d})})();TimeoutError=$extends(Error,{message:"timeout expired"});__=Promise.coerce=function(a){return a instanceof Promise?a:a instanceof Function?new Promise(function(b){b(a())}):Promise.resolve(a)};__.noop=function(){return Promise.resolve()};__.identity=function(a){return Promise.resolve(a)};
__.constant=function(a){return function(){return Promise.resolve(a)}};__.reject=function(a){return Promise.reject(a)};__.rejects=function(a){return function(){return Promise.reject(a)}};__.safe=function(a){return function(){try{return Promise.resolve(a.apply(this,arguments))}catch(b){return Promise.reject(b)}}};__.map=__.safe(function(a,b){if(_.isStrictlyObject(a)){var c=_.coerceToEmpty(a),d=[];_.each2(a,function(a,f){d.push(__.identity(b(a,f)).then(function(a){c[f]=a}))});return __.all(d).then(_.constant(c))}return b(a)});
__.then=function(a,b){return __(a).then(_.coerceToFunction(b))};__.seq=function(a){return __(_.reduce2(a,__.then))};__.all=function(a){return Promise.all(a)};__.delay=function(a){return __.delays(a)()};__.delays=function(a){return function(b){return new Promise(function(c){setTimeout(function(){c(b)},a||0)})}};$mixin(Array,{race:$property(function(){return Promise.race(this)})});
...
$mixin(Function,{promisifyAll:$static(function(a,b){var c=new Set((b||{}).except||[]);return _.map2(a,function(b,e){if(b instanceof Function){var f=b.bind(a);return c.has(e)?f:f.promisify}return b})}),promisify:$hidden($property(function(){var a=this;return function(){var b=this,c=arguments;return new Promise(function(d,e){a.apply(b,_.asArray(c).concat(function(a,b){a&&e(a);d(b)}))})}}))});$platform.NodeJS&&($global.requirePromisified=function(a,b){return Function.promisifyAll(require(a),b)});
$mixin(Set,{copy:$property(function(){return new Set(this)}),items:$property(function(){return Array.from(this.values())}),contains:$property(function(){var a=this;return function(b){return a.has(b)}}),extend:function(a){a=$jscomp.makeIterator(a);for(var b=a.next();!b.done;b=a.next())this.add(b.value);return this},extended:function(a){return this.copy.extend(a)}});
