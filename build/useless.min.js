...
k=b._onAfter,l=b._intercept,b=d||this,m,n=void 0;if(g.length){m=0;for(n=g.length;m<n;m++)g[m].apply(b,arguments);g.removeAll()}m=0;for(n=h.length;m<n;m++)h[m].apply(b,arguments);g=(l.length?_.cps.compose([a].concat(l)):a).apply(b,arguments);if(k.length||f.length){h=_.asArray(arguments).concat(g);m=0;for(n=k.length;m<n;m++)k[m].apply(b,h);if(f.length){m=0;for(n=f.length;m<n;m++)f[m].apply(b,h);f.removeAll()}}return g}))})})})();
