...
e.writeFileSync(a,b,f);c()}):API.post("source/"+a,_.extend2({},this.apiConfig,{what:{text:b},failure:UI.error,success:function(){log.ok(a,"\u2014 successfully saved");c&&c()}}))}});_.readSourceLine=SourceFiles.line;_.readSource=SourceFiles.read;_.writeSource=SourceFiles.write;
...
a.replace($sourcePath,"");return Platform.NodeJS&&"/"!==a[0]||0<=b.indexOf("/node_modules/")||0<=a.indexOf("/node_modules/")&&!b||0<=b.indexOf("underscore")||0<=b.indexOf("jquery")}),fromRawString:$static(_.sequence(function(a){return CallStack.rawStringToArray(a)},function(a){return _.map(a,function(a){return _.extend(a,{calleeShort:_.last(a.callee.split(".")),fileName:_.last(a.file.split("/")),fileShort:CallStack.shortenPath(a.file),thirdParty:CallStack.isThirdParty(a.file)})})},function(a){return _.map(a,
function(a){a.source="";a.sourceReady=_.barrier();_.readSourceLine(a.file,a.line-1,function(c){a.source=c;a.sourceReady(c)});return a})},function(a){return CallStack.fromParsedArray(a)})),rawStringToArray:$static(function(a){a=_.rest((a||"").split("\n"),"browser"===_.platform().engine?1:0);return _.map(a,function(a){a=a.trimmed;var c,d=[],e=a.match(/at (.+) \((.+)\)/);e?(c=e[1],d=_.rest(e[2].match(/(.*):(.+):(.+)/)||[])):(e=a.match(/at (.+)/))&&e[1]&&(d=_.rest(e[1].match(/(.*):(.+):(.+)/)||[]));return{beforeParse:a,
callee:c||"",file:d[0]||"",line:(d[1]||"").integerValue,column:(d[2]||"").integerValue}})})});$prototype.macro(function(a,b){var c=CallStack.currentAsRawString;a.$meta||(a.$meta=$static(_.cps.memoize(function(a){_.cps.find(CallStack.fromRawString(c).reversed,function(a,b){a.sourceReady(function(c){c=(c||"").match(/([A-z]+)\s*=\s*\$(prototype|singleton|component|extends|trait|aspect)/);b(c&&{name:c[1],type:c[2],file:a.fileShort}||!1)})},function(b){a(b||{})})})));return a});
_.measure=function(a,b){if(b){var c=_.now();a(function(){b(_.now()-c)})}else return c=_.now(),a(),_.now()-c};_.perfTest=function(a,b){var c=_.isFunction(a)?{test:a}:a,d={};_.cps.each(c,function(a,b,c){var h=[],k=function(){for(var c=0;500>c;c++)h.push(a());console.log(b,h)};k();_.delay(function(){d[b]=_.measure(k)/500;c()},100)},function(){b(d)})};_.defineTagKeyword("shouldFail");_.defineTagKeyword("async");_.defineTagKeyword("assertion");
