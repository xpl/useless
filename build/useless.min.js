...
$prototype.macro(function(a,b){a.$meta||(a.$meta=$static(_.cps.memoize($prototype.impl.findMeta(CallStack.currentAsRawString))));return a});_.hasLog=!0;_.extend(log=function(){return log.write.apply(this,[log.config({location:!0,stackOffset:1})].concat(_.asArray(arguments)))},{Config:$prototype(),config:function(a){return new log.Config(a)}});
...
return a}));this.run=this.$(this.run)},run:_.interlocked(function(a,b,c){var d=3===arguments.length?c:_.identity,e=this.runConfig=_.extend({suites:[],silent:!0,verbose:!1,timeout:2E3,filter:_.identity,testStarted:function(a){},testComplete:function(a){}},b),f=_.isArray(e.suites),g=_.map(e.suites,this.$(function(a,b){return this.testSuite(f?a.name:b,f?a.tests:a,e.context,a.proto)}));(!1===e.codebase?_.cps.constant([]):this.$(this.collectPrototypeTests))(this.$(function(b){var c=!1===e.codebase?[]:
this.collectTests();b=_.flatten(_.pluck(c.concat(g).concat(b),"tests"));b=_.filter(b,e.shouldRun||_.constant(!0));this.runningTests=_.map(b,function(a,b){return _.extend(a,{indent:e.indent,index:b})});_.assertTypeMatches(_.map(_.pluck(this.runningTests,"routine"),$untag),["function"]);this.runningTests=_.filter(this.runningTests,e.filter||_.identity);_.cps.each(this.runningTests,this.$(this.runTest),this.$(function(){_.assert(!0!==e.done);e.done=!0;this.printLog(e);this.failedTests=_.filter(this.runningTests,
_.property("failed"));this.failed=0<this.failedTests.length;d(!this.failed);a()}))}))}),onException:function(a){if(this.currentAssertion)this.currentAssertion.onException(a);else throw a;},defineAssertions:function(a){_.each(a,function(a,c){this.defineAssertion(c,a)},this)},runTest:function(a,b,c){var d=this,e=this.runConfig;log.impl.configStack=[];d.toCPS(e.testStarted)(a,function(b){b=b||_.noop;a.verbose=e.verbose;a.timeout=e.timeout;a.startTime=Date.now();a.run(function(){d.toCPS(e.testComplete)(_.extend(a,
{time:Date.now()-a.startTime}),function(){b();c()})})})},toCPS:function(a){return 2===_.numArgs(a)?a:function(b,c){a(b);c()}},collectTests:function(){return _.map(_.tests,this.$(function(a,b){return this.testSuite(b,a)}))},collectPrototypeTests:function(a){_.cps.map(this.prototypeTests,this.$(function(a,c){a.proto.$meta(this.$(function(d){c(this.testSuite(d.name,a.tests,void 0,a.proto))}))}),a)},testSuite:function(a,b,c,d){return{name:a||"",tests:_(_.pairs("function"===typeof b&&_.object([[a,b]])||
b)).map(function(b){var f=new Test({proto:d,name:b[0],routine:b[1],suite:a,context:c});f.complete(function(){(f.hasLog=0<f.logCalls.length)||(f.failed?log.red("FAIL"):f.verbose&&log.green("PASS"))});return f})}},defineAssertion:function(a,b){var c=this;_.deleteKeyword(a);_.defineKeyword(a,Tags.modify(b,function(d){return _.withSameArgs(d,function(){var e=$callStack.safeLocation($platform.Browser&&!$platform.Chrome?0:1);return c.currentAssertion?c.currentAssertion.babyAssertion(a,b,d,arguments,e):
d.apply(c,arguments)})}))},printLog:function(a){if(!a.supressLog){var b=_.filter(this.runningTests,function(b){return b.failed||!a.silent&&b.hasLog}),c=_.filter(this.runningTests,_.property("failed"));_.invoke(a.verbose?this.runningTests:b,"printLog");c.length?log.orange("\n"+log.boldLine+"\nSOME TESTS FAILED:",_.pluck(c,"name").join(", "),"\n\n"):!0!==a.silent&&log.green("\n"+log.boldLine+"\nALL TESTS PASS\n\n")}}});
...
function(d){b.complete(d.arity0);a&&b.complete(a);0<c.length?c.call(b.context,b.$(b.finalize)):(d=c.call(b.context),_.isArrayLike(d)&&d[0]instanceof Promise&&(d=__.all(d)),d instanceof Promise?d.then(function(a){b.finalize()}.postponed,function(a){b.onException(a)}):b.finalize())})},printLog:function(){var a=this.suite&&this.suite!==this.name&&(this.suite||"").quote("[]")||"";log.write(log.color.blue,"\n"+log.boldLine,"\n"+_.nonempty([a,this.name]).join(" "),(this.index+" of "+Testosterone.runningTests.length).quote("()")+
...
a.parse(e),k=h.name&&h.name.decapitalized||e;if(h=h.how&&_["on"+h.how.capitalized]||_.intercept){var l=b[k];l&&_.isFunction(l)&&h(b,k,c);var l=b.prototype,m=l[k];m&&_.isFunction(m)&&h(l,k,c)}}});b.aspectAdded&&b.aspectAdded(d);return d})})();TimeoutError=$extends(Error,{message:"timeout expired"});__=Promise.coerce=function(a){var b=this,c=_.rest(arguments);return a instanceof Promise?a:a instanceof Function?new Promise(function(d){d(a.apply(b,c))}):Promise.resolve(a)};__.noop=function(){return Promise.resolve()};
__.identity=function(a){return Promise.resolve(a)};__.constant=function(a){return function(){return Promise.resolve(a)}};__.reject=function(a){return Promise.reject(a)};__.rejects=function(a){return function(){return Promise.reject(a)}};
__.scatter=function(a,b){return __.then(a,function(a){if(_.isStrictlyObject(a)){var d=_.coerceToEmpty(a),e=[];_.each2(a,function(f,g){e.push(__(b,f,g,a).then(function(a){a instanceof Array?d[a.second]=a.first:void 0!==a&&(d instanceof Array?d.push(a):d[g]=a)}))});return __.all(e).then(_.constant(d))}return __(b,a,void 0,a).then(function(a){return a instanceof Array?a.first:a})})};__.map=function(a,b){return __.scatter(a,__.$(b))};
__.filter=function(a,b){return __.scatter(a,function(a,d,e){return __(b,a,d,e).then(function(b){return!1===b?void 0:!0===b?a:b})})};__.each=function(a,b){return __.then(a,function(a){return new Promise(function(d,e){_.cps.each(a,function(a,c,d){__(b(a,c)).then(d)["catch"](e)},d)})})};__.then=function(a,b){return __(a).then(_.coerceToFunction(b))};__.seq=function(a){return __(_.reduce2(a,__.then))};__.all=function(a){return Promise.all(a)};__.delay=function(a){return __.delays(a)()};
__.delays=function(a){return function(b){return new Promise(function(c){setTimeout(function(){c(b)},a||0)})}};$mixin(Array,{race:$property(function(){return Promise.race(this)})});
...
return b})},done:function(a){return this.then(function(b){return a(null,b)},function(b){a(b,null);throw b;})},"finally":function(a){return this.then(function(b){return a(null,b)},function(b){return a(b,null)})},assert:function(a){return this.then(function(b){$assert(b,a);return b})},assertTypeMatches:function(a){return this.then(function(b){$assertTypeMatches(b,a);return b})},assertRejected:function(a){var b=0<arguments.length;return this["catch"](function(c){b&&$assert(c,a);return c})},panic:$property(function(){return this["catch"](function(a){($global.Panic||
$global.log)(a);throw a;})})});$mixin(Function,{promisifyAll:$static(function(a,b){b=b||{};var c=b.except||_.noop;c instanceof Array&&(c=c.asSet.matches);var d={},e;for(e in a){var f=a[e];f instanceof Function?(f=f.bind(a),d[e]=c(e)?f:f.promisify):d[e]=f}return d}),promisify:$hidden($property(function(){var a=this;return function(){var b=this,c=arguments;return new Promise(function(d,e){a.apply(b,_.asArray(c).concat(function(a,b){a&&e(a);d(b)}))})}}))});$mixin(Array,{asSet:$property(function(){return new Set(this)})});
$mixin(Set,{copy:$property(function(){return new Set(this)}),asArray:$property(function(){return Array.from(this.values())}),matches:$property(function(){var a=this;return function(b){return a.has(b)}}),extend:function(a){a=$jscomp.makeIterator(a);for(var b=a.next();!b.done;b=a.next())this.add(b.value);return this},extended:function(a){return this.copy.extend(a)}});
