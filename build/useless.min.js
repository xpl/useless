...
function(b){return $alias.matches(b)?a[Tags.unwrap(b)]:b})},flatten:function(a){var b=_.pick(a,this.isTagKeywordGroup),b=_.object(_.flatten(_.map(b,function(a,b){return _.map(a,function(a,c){return[c,Tags.add(b.slice(1),a)]})}),!0));a=_.omit(a,this.isTagKeywordGroup);return _.extend(a,b)},isTagKeywordGroup:function(a,b){var c=Tags.unwrap(a);return _.isKeyword(b)&&_.isTagKeyword(b)&&"object"===typeof Tags.unwrap(c)&&!_.isArray(c)}}});
_.hasAsserts&&_.defineKeyword("assertCallOrder",function(a){var b=1,c=[];return a.call(this,function(a){var e=b++;return $test(_.extend({$$uniqueTag$$:e},$prototype.mapMethods(a,function(a,b){return function(){c.push({ctx:this,tag:e,name:b});return a.apply(this,arguments)}})))},function(){var a=_.map(_.asArray(arguments),function(a){return{ctx:a[0],tag:a[1].$$uniqueTag$$,name:a[2]}}),a=_.zipZip(a,c,function(a,b){return _.nonempty(a!==b?[a,b]:[b])});_.every(a,function(a){a=_.isArray(a)?a[0]:a;return 1===
a.tag.length&&1===a.name.length&&1===a.ctx.length})||_.assertionFailed({notMatching:log.asTable(_.map(a,function(a,b){return{name:b+1+". "+(b>=c.length?"NOT CALLED":a.name.join(" \u2190 ")),"this":1<a.ctx.length&&"wrong"||"",proto:1<a.tag.length&&"wrong"||""}}))})})});_.isTraitOf=function(a,b){var c=b&&b.constructor;return c&&c.hasTrait&&c.hasTrait(a)||!1};_.isTypeOf=_.or(_.isTypeOf,_.isTraitOf);
...
Platform=$singleton({$property:{engine:_.platform().engine,system:_.platform().system,device:_.platform().device,touch:_.platform().touch||!1,NodeJS:"node"===_.platform().engine,iPad:"iPad"===_.platform().device,iPhone:"iPhone"===_.platform().device,iOS:"iOS"===_.platform().system}});_(["method","property","flipped"]).each(_.defineTagKeyword);
$extensionMethods=function(a,b){_.each(b,function(b,d){var e=Tags.unwrap(b);d in _||(_[d]=_[d]||e);if(b.$method||!b.$property&&!_.oneArg(e)||d in a.prototype){if(b.$property)throw Error("$extensionMethods: crazy input, unable to match");a.prototype[d]=_.asMethod(b.$flipped?_.flip(e):e)}else _.defineHiddenProperty(a.prototype,d,function(){return e(this)})})};
...
f>g?new Vec2(-l,0):new Vec2(0,-m),d.delta=b.scale(d.time),d.where=a.add(d.delta),d},clone:$property(function(){return new BBox(this.x,this.y,this.width,this.height)}),floor:$property(function(){return new Vec2(Math.floor(this.x),Math.floor(this.y))}),css:$property(function(){return{left:this.left,top:this.top,width:this.width,height:this.height}}),leftTop:$property(function(){return new Vec2(this.left,this.top)}),rightBottom:$property(function(){return new Vec2(this.right,this.bottom)}),left:$property(function(){return this.x-
...
this.y,this.width+a,this.height+a)},toString:function(){return"{"+this.x+","+this.y+":"+this.width+"\u00d7"+this.height+"}"}});
...
Platform.NodeJS&&(module.exports=_);_("bindable trigger triggerOnce barrier observable observableProperty memoize debounce throttle overrideThis".split(" ")).each(_.defineTagKeyword);_.defineKeyword("component",function(a){return $extends(Component,a)});$prototype.inheritsBaseValues=function(a){$prototype.macro(a,function(b,c,d,e){_.defaults(c,e&&e[a]);(b.$trait||b.$traits)&&_.each(b.$trait&&[b.$trait]||b.$traits,function(b){_.defaults(c,b[a])});b[a]=$static($builtin($property(_.constant(c))));return b})};
$prototype.inheritsBaseValues("$defaults");$prototype.inheritsBaseValues("$requires");
...
[];return this},destroyAll:function(){_.each(this.children_,function(a){a.parent_=void 0;a.destroy()});this.children_=[];return this}});_.defineTagKeyword("shouldFail");_.defineTagKeyword("async");_.defineTagKeyword("assertion");
...
function(a){log.impl.writeBackend(a)})}});Platform.NodeJS&&(module.exports=Testosterone);_.clamp=function(a,b,c){return Math.max(b,Math.min(c,a))};_.lerp=function(a,b,c){return b+(c-b)*a};_.rescale=function(a,b,c,d){a=(a-b[0])/(b[1]-b[0]);return _.lerp(d&&d.clamp?_.clamp(a,0,1):a,c[0],c[1])};
Vec2=$prototype({$static:{zero:$property(function(){return new Vec2(0,0)}),unit:$property(function(){return new Vec2(1,1)}),one:$alias("unit"),fromLT:function(a){return new Vec2(a.left,a.top)},fromLeftTop:$alias("fromLT"),dot:function(a,b){return a.x*b.x+a.y*b.y},lerp:function(a,b,c){return new Vec2(_.lerp(a,b.x,c.x),_.lerp(a,b.y,c.y))},clamp:function(a,b,c){return new Vec2(_.clamp(a.x,b.x,c.x),_.clamp(a.y,b.y,c.y))}},constructor:function(a,b){this.x=a;this.y=1===arguments.length?a:b},length:$property(function(){return Math.sqrt(this.x*
this.x+this.y*this.y)}),add:function(a){return new Vec2(this.x+a.x,this.y+a.y)},sub:function(a){return new Vec2(this.x-a.x,this.y-a.y)},scale:function(a,b){return new Vec2(this.x*a,this.y*(void 0===b?a:b))},mul:function(a){return new Vec2(this.x*a.x,this.y*a.y)},divide:function(a){return new Vec2(this.x/a.x,this.y/a.y)},normal:$property(function(){return this.scale(1/this.length)}),perp:$property(function(){return new Vec2(this.y,-this.x)}),half:$property(function(){return new Vec2(.5*this.x,.5*this.y)}),
inverse:$property(function(){return new Vec2(-this.x,-this.y)}),cssLeftTop:$property(function(){return{left:Math.floor(this.x),top:Math.floor(this.y)}}),cssLeftTopMargin:$property(function(){return{marginLeft:Math.floor(this.x),marginTop:Math.floor(this.y)}}),cssWidthHeight:$property(function(){return{width:Math.floor(this.x),height:Math.floor(this.y)}}),floor:$property(function(){return new Vec2(Math.floor(this.x),Math.floor(this.y))}),sum:$static(function(a){return _.reduce(_.isArray(a)&&a||_.asArray(arguments),
function(a,c){return a.add(c||Vec2.zero)},Vec2.zero)}),toString:function(){return"{"+this.x+","+this.y+"}"}});
...
begin:$property("^"),end:$property("$"),space:$property("\\s"),anything:$property(".*"),something:$property(".+"),comma:$property(","),parentheses:function(a){return["\\(",a,"\\)"]},brackets:function(a){return["\\[",a,"\\]"]},isSubexpr:function(a){return _.isStrictlyObject(a)&&!_.isArray(a)?!0:!1},root:function(a){return a&&a.$$?a.$$:a},initDSL:function(){_.defineKeyword("r",function(){return $$r([])});_.defineKeyword("$r",function(a){var b=function(b){a.push(b);return a.forward};_.defineHiddenProperty(a,
"then",function(b){a.push(R.root(b));return a});_.defineHiddenProperty(a,"text",function(b){a.push(R.text(b));return a});_.defineHiddenProperty(a,"expr",function(b,d){a.push(R.subexpr(b,R.root(d)));return a});_.defineHiddenProperty(a,"forward",function(){return a.next||((a.next=$r).prev=a).next});_.each(["maybe","anyOf","someOf","oneOf","except"],function(c){_.defineHiddenProperty(a,c,function(){return b(R[c](a.forward))})});_.each(["parentheses","brackets"],function(b){_.defineHiddenProperty(a,"in"+
b.capitalized,function(){return a.$$.prev=$$r(R[b](a.$$))})});_.each(["or"],function(b){_.defineHiddenProperty(a,b,function(){var d=$r;return(d.prev=a.$$.prev=$$r(R[b](a.$$,d))).next=d})});_.each(["begin","end","space","anything","something"],function(c){_.defineHiddenProperty(a,c,function(){return b([R[c],a.forward])})});_.defineHiddenProperty(a,"$$",function(){for(var b=a;b.prev;)b=b.prev;return b});_.defineHiddenProperty(a,"$",function(){return R.expr(a.$$)});return a})}});
