...
a('<ul class="callstack">').append(_.map(c,this.$(function(b){var c=a('<li class="callstack-entry">').toggleClass("third-party",b.thirdParty).toggleClass("native",b["native"]).append([a('<span class="file">').text(_.nonempty([b.index?"(index)":b.fileShort,b.line]).join(":")),a('<span class="callee">').text(b.calleeShort),a('<span class="src i-am-busy">').click(this.$(function(d){var g=a(d.delegateTarget);g.waitUntil(_.readSource.partial((b.remote?"api/source/":"")+b.file),this.$(function(d){c.is(".full")?
(c.removeClass("full"),c.transitionend(function(){c.is(".full")||b.sourceReady(g.$(a.fn.text))})):(c.addClass("full"),g.html(_.map(d.split("\n"),function(b){return a('<div class="line">').text(b)})),d=g.find(".line").eq(b.line-1).addClass("hili"),d.length&&(d=d.offset().top-g.offset().top,g.scrollTop(d-100)),_.delay(this.$(function(){var a=g.outerBBox().bottom+242-this.modalBody.outerBBox().bottom;0<a&&this.modalBody.animate({scrollTop:this.modalBody.scrollTop()+a},250)})))}))}))]);b.sourceReady(function(a){c.find(".src").removeClass("i-am-busy").text(a)});
return c})))]}});a.fn.extend({enableScrollFaders:function(b){var c=b&&b.horizontal,d,e;b=this.find(b&&b.scroller||".scroller");this.css({position:"relative"});this.append(d=a('<div class="scroll-fader scroll-fader-'+(c?"left":"top")+'"></div>')).append(e=a('<div class="scroll-fader scroll-fader-'+(c?"right":"bottom")+'"></div>'));b.scroll(function(){var b=c?a(this).scrollLeft():a(this).scrollTop(),h=c?a(this).width():a(this).height(),g=(c?this.scrollWidth:this.scrollHeight)-1;d.css({opacity:0<b?1:
0});e.css({opacity:b+h<g?1:0})}).scroll();return this}})})(jQuery);
