...
b.filename+":"+b.lineno+":"+b.colno})))});var d=function(b,d){return __supressErrorReporting=function(){var k={name:name,stack:Error().stack,asyncContext:c},f=_.asArray(arguments),h=f[d];h.__uncaughtJS_wrapper=f[d]=__supressErrorReporting=function(){c=k;try{return h.apply(this,arguments)}catch(b){a(_.extend(b,{asyncContext:k}))}};return b.apply(this,f)}};window.setTimeout=d(window.setTimeout,0);b(function(a){return d(a,1)},function(a){return function(b,c,d,h){return a.call(this,b,c.__uncaughtJS_wrapper||
...
c=log.cleanArgs(b),d=_.extend({indent:0},a,log.readConfig(b)),e=Platform.NodeJS?3:2,g=(log.impl.writeBackend.indent||0)+d.indent,k=log.impl.stringifyArguments(c,d),g=_.times(g,_.constant("\t")).join(""),k=k.reversed.match(/(\n*)([^]*)/),d=d.location&&log.impl.location(d.where||$callStack[e+(d.stackOffset||0)])||"",b={color:log.readColor(b),indentedText:k[2].reversed.split("\n").map(_.prepends(g)).join("\n"),trailNewlines:k[1],codeLocation:d};log.impl.writeBackend(b);return c[0]})},defaultWriteBackend:function(a){var b=
...
maxTotalWidth:0},k=_.reduce(e,_.sum,0),f=_.map(e,_.muls(1/k)),h=Math.max(0,k-g.maxTotalWidth),l=_.map(e,function(a,b){return Math.max(g.minColumnWidths[b],Math.floor(a-h*f[b]))}),d=d.map(function(a){return[l,a].zip(_.subtract)});return[c,d].zip(_.zap.tails(function(a,b){return 0<=b?a+" ".repeats(b):_.initial(a,-b).join("")}).then(_.joinsWith("  ")))}});Platform.NodeJS&&(module.exports=log);_.defineTagKeyword("shouldFail");_.defineTagKeyword("async");_.defineTagKeyword("assertion");
