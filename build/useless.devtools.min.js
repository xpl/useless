...
function(){var h={name:name,stack:Error().stack,asyncContext:c},f=_.asArray(arguments),k=f[d];f[d]=_.extendWith({__uncaughtJS_wraps:k},__supressErrorReporting=function(){c=h;try{return k.apply(this,arguments)}catch(b){a(_.extend(b,{asyncContext:h}))}});return b.apply(this,f)}};window.setTimeout=d(window.setTimeout,0);b(function(a){return d(a,1)},function(a){return function(b,c,d,k){return a.call(this,b,c.__uncaughtJS_wraps||c,d)}})}})();_.hasReflection=!0;
...
log.config({location:g.location,where:g.location}));g.evalLogCalls();e.endAssertion(g)}):e.endAssertion(g)});else{e.beginAssertion(g);try{var a=c.apply(e.context,d);e.endAssertion(g);return a}catch(f){e.onException(f),e.endAssertion(g)}}})},beginAssertion:function(a){a.async&&(Testosterone.currentTest=a);this.assertionStack([a].concat(this.assertionStack.value))},endAssertion:function(a){Testosterone.currentTest===a&&(Testosterone.currentTest=this);if(a.shouldFail&&!a.failed)this.onException(_.assertionError({notMatching:"not failed (as should)"}));
...
"").click(this.$(function(b){a(b.delegateTarget).parent().toggleClass("all").transitionend(this.$(function(){this.modalBody.scroll()}))})),a('<ul class="callstack">').append(_.map(c,this.$(function(b){var c=a('<li class="callstack-entry">').toggleClass("third-party",b.thirdParty).toggleClass("native",b["native"]).append([a('<span class="file">').text(_.nonempty([b.index?"(index)":b.fileShort,b.line]).join(":")),a('<span class="callee">').text(b.calleeShort),a('<span class="src i-am-busy">').click(this.$(function(d){var f=
a(d.delegateTarget);f.waitUntil(_.readSource.partial((b.remote?"api/source/":"")+b.file),this.$(function(d){c.is(".full")?(c.removeClass("full"),c.transitionend(function(){c.is(".full")||f.text(b.source)})):(c.addClass("full"),f.html(_.map(d.split("\n"),function(b){return a('<div class="line">').text(b)})),d=f.find(".line").eq(b.line-1).addClass("hili"),d.length&&(d=d.offset().top-f.offset().top,f.scrollTop(d-100)),_.delay(this.$(function(){var a=f.outerBBox().bottom+242-this.modalBody.outerBBox().bottom;
