...
g={encoding:"utf8"};try{e.mkdirSync(a+".backups")}catch(h){}e.writeFileSync(a+".backups/"+Date.now(),d,g);e.writeFileSync(a,b,g);c()}):API.post("source/"+a,_.extend2({},this.apiConfig,{what:{text:b},failure:Panic,success:function(){log.ok(a,"\u2014 successfully saved");c&&c()}}))}});_.readSourceLine=SourceFiles.line;_.readSource=SourceFiles.read;_.writeSource=SourceFiles.write;
...
log.impl.defaultWriteBackend},impl:{write:function(a){return $restArg(function(){var b=log.writeBackend(),c=_.asArray(arguments),d=log.cleanArgs(c),e=_.extend({indent:0},a,log.readConfig(c)),g=(b.indent||0)+e.indent,h=log.impl.stringifyArguments(d,e),g=_.times(g,_.constant("\t")).join(""),h=h.reversed.match(/(\n*)([^]*)/),f=e.location&&log.impl.location(e.where||$callStack[3+(e.stackOffset||0)])||"",c={color:log.readColor(c),indentedText:h[2].reversed.split("\n").map(_.prepends(g)).join("\n"),trailNewlines:h[1],
codeLocation:f,config:e};b(c);return d[0]})},defaultWriteBackend:function(a){var b=a.color,c=a.indentedText,d=a.codeLocation;a=a.trailNewlines;(b=b&&(Platform.NodeJS?b.shell:b.css))?Platform.NodeJS?console.log(b+c+"\u001b[0m",d,a):(c=c.split("\n"),c=[_.first(c)||""].concat(_.rest(c).map(_.prepends(" "))),console.log("%c"+c.join("\n"),"color: "+b,d,a)):console.log(c,d,a)},location:function(a){return _.quoteWith("()",_.nonempty([a.calleeShort,a.fileName+":"+a.line]).join(" @ "))},stringifyArguments:function(a,
...
maxTotalWidth:0},h=_.reduce(e,_.sum,0),f=_.map(e,_.muls(1/h)),k=Math.max(0,h-g.maxTotalWidth),l=_.map(e,function(a,b){return Math.max(g.minColumnWidths[b],Math.floor(a-k*f[b]))}),d=d.map(function(a){return[l,a].zip(_.subtract)});return[c,d].zip(_.zap.tails(function(a,b){return 0<=b?a+" ".repeats(b):_.initial(a,-b).join("")}).then(_.joinsWith("  ")))}});Platform.NodeJS&&(module.exports=log);_.defineTagKeyword("shouldFail");_.defineTagKeyword("async");_.defineTagKeyword("assertion");
...
b.line]).join(":")),a('<span class="callee">').text(b.calleeShort),a('<span class="src i-am-busy">').click(this.$(function(d){var f=a(d.delegateTarget);f.waitUntil(_.readSource.partial((b.remote?"api/source/":"")+b.file),this.$(function(d){c.is(".full")?(c.removeClass("full"),c.transitionend(function(){c.is(".full")||b.sourceReady(f.$(a.fn.text))})):(c.addClass("full"),f.html(_.map(d.split("\n"),function(b){return a('<div class="line">').text(b)})),d=f.find(".line").eq(b.line-1).addClass("hili"),
d.length&&(d=d.offset().top-f.offset().top,f.scrollTop(d-100)),_.delay(this.$(function(){var a=f.outerBBox().bottom+242-this.modalBody.outerBBox().bottom;0<a&&this.modalBody.animate({scrollTop:this.modalBody.scrollTop()+a},250)})))}))}))]);b.sourceReady(function(a){c.find(".src").removeClass("i-am-busy").text(a)});return c})))]}});a.fn.extend({enableScrollFaders:function(b){var c=b&&b.horizontal,d,e;b=this.find(b&&b.scroller||".scroller");this.css({position:"relative"});this.append(d=a('<div class="scroll-fader scroll-fader-'+
(c?"left":"top")+'"></div>')).append(e=a('<div class="scroll-fader scroll-fader-'+(c?"right":"bottom")+'"></div>'));b.scroll(function(){var b=c?a(this).scrollLeft():a(this).scrollTop(),h=c?a(this).width():a(this).height(),f=(c?this.scrollWidth:this.scrollHeight)-1;d.css({opacity:0<b?1:0});e.css({opacity:b+h<f?1:0})}).scroll();return this}})})(jQuery);
