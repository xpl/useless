...
Note the letter **'s'** at the end the function name (_.applie**\[s\]**). This is conventional naming for all such *"wrapping"* functions.The letter *'s'* indicates that the function is not called immediately, but generates another function instead, which "doe**[s]** something" later upon its turn.
